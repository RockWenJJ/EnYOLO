...
from enyolo.utils import is_metainfo_lower
...
def parse_args():
    ...
    parser.add_argument(
        '--mode', type=str, default='detection',
        help='the mode for testing, could chose detection or restoration')
    parser.add_argument(
        '--out',
        type=str,
...
def main():
    ...
    # start testing
    if args.mode == 'detection': # by default
        runner.test()
    elif args.mode == 'restoration':
        from tqdm import tqdm
        results = runner.test_res()
        print("Save Predictions...")
        # save images
        out_dir = os.path.join(cfg.work_dir, 'res_results')
        os.makedirs(out_dir)
        for result in tqdm(results):
            out_name = os.path.join(out_dir, result['img_name'])
